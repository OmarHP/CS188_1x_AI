...
def cornersHeuristic(state, problem):
    ...
    for x in range(0, 3):
        if(stateCorners[x]==False):
            distances.append(manhattanDistance(currentPosition,corners[x]))
    ...
    for x in range (0,foodGrid.width):
        for y in range(0,foodGrid.height):
            if(foodGrid[x][y]==True):
                distances.append(manhattanDistance(position,(x,y)))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState):
        ...
        #Calcutating the closest food
        distances=list()
        from util import manhattanDistance
        for x in range (0,food.width):
            for y in range(0,food.height):
                if(food[x][y]==True):
                    distances.append(((x,y),manhattanDistance(startPosition,(x,y))))

        distances=sorted(distances,key=lambda x: x[1])
        closestFood=distances.pop(0)[0]
            ...
            if(current[0] not in explored):
                ...
                if(problem.isGoalState(currentState)):
                    ...
                    for successor in successors:
                        action=successor[1]
                        newPath=list(current[1])
                        newPath.append(action)
                        cost=current[2]+successor[2]
                        heuristicCost=manhattanDistance(successor[0],closestFood)
                        fringe.push((successor[0],newPath,cost),heuristicCost)
